...
grep -r -E "(src=|url\(|background-image|\.png|\.jpg|\.jpeg|\.gif|\.webp|\.svg)" --include="*.js" --include="*.jsx" --include="*.ts" --include="*.tsx" --include="*.css" --include="*.scss" --include="*.html" --include="*.json" . > /tmp/image_references.txt 2>/dev/null || true
